...
def add_basic_to_object(object , n = N):
    ...
    for index, label  in enumerate(object.label):  # index = 0 means it adds one to 2nd row
        ...
        if label > 0:
            ...
            temp_state = []
            for  j , content  in enumerate(starting_content):
            ...
            for obj in final['Objects']:
                for state in obj.contents[tuple(content_of_new)]:
...
def add_basic_to_object2(object , n = N):
    new_state = State( None  )
    new_state.add_dimension(1 , [object.top_state , basic.top_state ])

    new_label = list(object.label)
    new_label[0] +=1
    state_index = 0
    new_obj = Object([new_state] , new_label)
    new_obj.fill_2(object , basic)
    final = {'Objects': []}
    final['Objects'].append(new_obj)
    if n == state_index:
        return final['Objects'][n]
    for index, label  in enumerate(object.label):            ########## index = 0 means it adds one to 2nd row
        ...
        if label > 0:
            ...
            loop_state = Ladder2(new_state, 0 , index + 1 , [object , basic])
            content_of_new = list(loop_state.content)
            ...
            for  j , content  in enumerate(starting_content):
                for state in object.contents[tuple(content)]:
                    temp_state.append(State([Base(1, [state , basic.contents[tuple(vectors[j])][0]])]))
            prev_states = []
            for obj in final['Objects']:
                for state in obj.contents[tuple(content_of_new)]:
                    prev_states.append(state)
            ...
            newest_obj = Object([newer_state] , new_label)
            newest_obj.fill_2(object , basic)
...
def add_object_to_object(obj1 , obj2 , n = None ):
    ...
    for key in true_key_list:
        try:
            ...
            print("ERROR; key in new_key_list that's  not in dict_of_contents")
    for key in true_key_list:
        try:
    ...
    return [tot , dict_of_contents]
    return tot

def add_object_to_object2(obj1 , obj2 , n = None):
    tot = {'Objects' : []}
    dict_of_contents = {}
    for content1 in obj1.contents:
        for content2 in obj2.contents:
            ...
            for state1 in obj1.contents[content1]:
                for state2 in obj2.contents[content2]:
                    produced_state = State(None)
                    produced_state.add_dimension(1 , [state1, state2])
                    pos_bases.append(produced_state)
    ...
    for key in dict_of_contents:
        key_list.append(key)
        print(str(dict_of_contents[key]["mult"]) + "·" + str(key) + ", ")

    state_index = 0
    new_state = State(None)
    new_state.add_dimension(1 , [obj1.top_state , obj2.top_state])
    new_label = content_to_label(new_state.content)
    new_obj = Object([new_state] , new_label)
    tot['Objects'].append(new_obj)
    if n == state_index:
        return tot['Objects'][n]
    true_key_list = new_obj.fill_( obj1 , obj2 , True )
    for key in true_key_list:
        try:
            dict_of_contents[key]["mult"]-=1
        except:
    ...
    for key in true_key_list:
        try:
            if dict_of_contents[key]["mult"] == 1:
                ...
                newer_label = content_to_label(newer_state.content)
                newest_obj = Object([newer_state] , newer_label)
                new_key_list = newest_obj.fill_( obj1 , obj2 , True)
                tot['Objects'].append(newest_obj)
                for key2 in new_key_list:
                    try:
                        dict_of_contents[key2]["mult"]-=1
                ...
                mult = dict_of_contents[key]["mult"]
                base_states = deepcopy(dict_of_contents[key]['bases'])
                prev_states = []
                for obj in tot['Objects']:
                    for state in obj.contents[key]:
                        prev_states.append(state)
                orthnorm_states  = gramschmidt2(prev_states + base_states)[0]
                new_states = orthnorm_states[-mult:]
                for newer_state in new_states:
                    newer_state.normalize()
                    newer_label = content_to_label(newer_state.content)
...
def build_object2(label):
    ...
    for index, length in enumerate(label):  #each index creates a 2 dimensional structure hence 2 more indices
        ...
        for j in range(index + 1):
            if j==0:
                ...
                temp_temp_obj = temp_obj
                temp_obj = add_basic_to_object2(temp_obj , 1)
                temp_obj.expand_tensor_products()
    ...
    for i , obj in enumerate(obj_list):
        if final == None:
            ...
            temp_final = final
            final = add_object_to_object2(final , obj , 0)
            if i < len(obj_list)-1:
                final.fill_2(temp_final , obj)


    final.fill_2(temp_final , obj , skip_ladderfill = True)
    return [final , temp_final , obj]
